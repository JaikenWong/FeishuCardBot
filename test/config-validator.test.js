...
test('缺工具与字段报错', () => {
  const schema = { submit: { text: '✅ 确定创建' }, fields: [{ type: 'select', required: true }] }
...
test('submit.text 过长时报错', () => {
  const schema = { submit: { text: 'x'.repeat(31) }, fields: [{ name: 'material_name', type: 'input', required: true }] }
  const out = validateAgentRuntimeConfig({
    schema,
    agentConfig: {
      allowedTools: ['list_field_options', 'prepare_create_part'],
...
test('submit.action 非法时报错', () => {
  const schema = { submit: { text: '✅ 确定创建', action: 'bad_action' }, fields: [{ name: 'material_name', type: 'input', required: true }] }
  const out = validateAgentRuntimeConfig({
    schema,
    agentConfig: {
      allowedTools: ['list_field_options', 'prepare_create_part'],
...
test('fields 含空项时不崩溃并报错', () => {
  ...
  assert.strictEqual(out.ok, false)
  assert.ok(out.errors.some((e) => e.includes('form-schema 至少要有一个 required 字段')))
  assert.ok(out.errors.some((e) => e.includes('form-schema 字段缺少 name')))
})
