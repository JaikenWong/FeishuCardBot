    ...
    if (f.type === 'select') {
      ...
      if (src.type === 'static' && Array.isArray(src.options) && src.options.length > 0) {
        ...
        if (badTextOptions.length > 0) {
          errors.push(`schema 字段 ${f.name} optionSource.options.text 非法`)
        }
        ...
        if (badValueOptions.length > 0) {
          errors.push(`schema 字段 ${f.name} optionSource.options.value 非法`)
        }
        ...
        if (dupValues.length > 0) {
          errors.push(`schema 字段 ${f.name} optionSource.options.value 重复: ${Array.from(new Set(dupValues)).join(', ')}`)
        }
      }
    }
  }
